...
async function searchWeather() {
  ...
  const selectedDateInput = document.getElementById('dateInput').value; // ISO beklenir
  ...
  try {
    ...
    // Horizon'ı seçilen tarihi kapsayacak şekilde büyüt
    ...
    const requestData = {
      lat: location.lat,
      lon: location.lon,
      target_date: selectedISO.replace(/-/g, ''), // YYYYMMDD (replaceAll yerine regex)
    ...
    // API 'tarih' alanı YYYY-MM-DD -> ISO ile eşleştir
    ...
    if (targetForecast) {
      ...
      const last  = forecastList.length ? forecastList[forecastList.length - 1].tarih : undefined; // at(-1) yerine fallback
...
/* Eğer HTML'de inline onclick kullanıyorsan ve script type="module" ise:
window.searchWeather = searchWeather;
window.toggleTheme = toggleTheme;
window.togglePlanner = togglePlanner;
window.toggleBackgroundType = toggleBackgroundType;
